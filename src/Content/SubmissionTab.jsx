import React from 'react';
import { Download, Eye, MessageCircle, Plus } from 'lucide-react';

function SubmissionTab() {
  // Sample submissions data
  const submissions = [
    {
      id: 1,
      designName: "Banking App Redesign",
      designerName: "Sarah Chen",
      hasComments: true,
      commentsCount: 3,
      submittedAt: "2 hours ago"
    },
    {
      id: 2,
      designName: "Modern Banking UI",
      designerName: "Alex Rodriguez", 
      hasComments: false,
      commentsCount: 0,
      submittedAt: "6 hours ago"
    },
    {
      id: 3,
      designName: "Clean Bank Interface",
      designerName: "Maya Patel",
      hasComments: true,
      commentsCount: 1,
      submittedAt: "1 day ago"
    },
    {
      id: 4,
      designName: "Minimalist Banking App",
      designerName: "John Smith",
      hasComments: false,
      commentsCount: 0,
      submittedAt: "2 days ago"
    }
  ];

  return (
    <div className="space-y-4">
      <div className="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200">
          <h3 className="text-lg font-semibold">Design Submissions</h3>
          <p className="text-sm text-gray-600 mt-1">{submissions.length} submissions received</p>
        </div>
        
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  Design Name
                </th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  Designer Name
                </th>
                <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
                  Download
                </th>
                <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
                  View
                </th>
                <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
                  Comments
                </th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  Submitted
                </th>
              </tr>
            </thead>
            <tbody className="bg-white divide-y divide-gray-200">
              {submissions.map((submission) => (
                <tr key={submission.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="font-medium text-gray-900">
                      {submission.designName}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-600">
                      {submission.designerName}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    <button className="inline-flex items-center px-3 py-1.5 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 transition-colors">
                      <Download className="w-4 h-4 mr-1" />
                      Download
                    </button>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    <button className="inline-flex items-center px-3 py-1.5 border border-blue-300 rounded-md text-sm font-medium text-blue-700 bg-blue-50 hover:bg-blue-100 transition-colors">
                      <Eye className="w-4 h-4 mr-1" />
                      View
                    </button>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    {submission.hasComments ? (
                      <button className="inline-flex items-center px-3 py-1.5 border border-green-300 rounded-md text-sm font-medium text-green-700 bg-green-50 hover:bg-green-100 transition-colors">
                        <MessageCircle className="w-4 h-4 mr-1" />
                        Comments ({submission.commentsCount})
                      </button>
                    ) : (
                      <button className="inline-flex items-center px-3 py-1.5 border border-gray-300 rounded-md text-sm font-medium text-gray-600 bg-gray-50 hover:bg-gray-100 transition-colors">
                        <Plus className="w-4 h-4 mr-1" />
                        Add Comment
                      </button>
                    )}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-500">
                      {submission.submittedAt}
                    </div>
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
        
        {/* Empty state when no submissions */}
        {submissions.length === 0 && (
          <div className="text-center py-12">
            <div className="text-gray-400 mb-2">
              <Eye className="w-12 h-12 mx-auto" />
            </div>
            <h3 className="text-lg font-medium text-gray-900 mb-1">No submissions yet</h3>
            <p className="text-gray-500">Design submissions will appear here once participants start uploading.</p>
          </div>
        )}
      </div>
    </div>
  );
}

export default SubmissionTab;